...
# --- NEW FULL-WIDTH SECTION FOR EXTRA INFO ---
st.markdown("---")  # separator line

st.subheader("Additional Info")

# Pull height, weight, and any other fields available
height = df.loc[df["pokedex_number"] == index, "height_m"].values[0] if "height_m" in df.columns else "Unknown"
weight = df.loc[df["pokedex_number"] == index, "weight_kg"].values[0] if "weight_kg" in df.columns else "Unknown"
base_exp = df.loc[df["pokedex_number"] == index, "base_experience"].values[0] if "base_experience" in df.columns else "Unknown"
capture_rate = df.loc[df["pokedex_number"] == index, "capture_rate"].values[0] if "capture_rate" in df.columns else "Unknown"

# Display in two columns for nice layout
...
with info_col2:
    st.markdown(f"**Base Experience:** {base_exp}")
